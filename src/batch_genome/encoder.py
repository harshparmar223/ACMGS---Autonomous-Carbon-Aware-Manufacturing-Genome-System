...
if __name__ == "__main__":
    ...
    print(f"First genome vec: {genome[0]}")
    print(f"Genome stats:")
    print(f"  Mean:  {genome.mean(axis=0)[:5]}... (showing first 5)")
    print(f"  Std:   {genome.std(axis=0)[:5]}... (showing first 5)")
    print(f"{'='*50}")
    
    # Test retrieval
    print(f"\nTesting batch retrieval...")
    test_genome = get_genome_by_batch_id("BATCH_0000")
    print(f"Retrieved genome for BATCH_0000: {test_genome[:5]}... (first 5 dims)")
